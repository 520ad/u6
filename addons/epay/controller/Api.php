...
class Api extends Controller
{
    ...
    public function default()
    {
        ...
        $orderData = Rechargeorder::where("orderid", $orderid)->find();
        if (!$orderData) {
            $this->error("订单号不存在");
        }else{
            $payTypes = "微信";
            if ($orderData['paytype'] == "alipay") {
                $payTypes = "支付宝";
            }elseif ($orderData['paytype'] == "qq") {
                $payTypes = "QQ";
            }
            $orderInfo = array(
                "title"    => $orderData['memo'],
                "code_url" => $orderData['payurl'],
                "amount" => $orderData['allocationamount'],
                "orderid" => $orderData['orderid'],
                "payType" => $payTypes
            );
        }
        $this->view->assign("title", "收银台");
        $this->view->assign("orderData", $orderInfo);
