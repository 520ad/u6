...
class Version extends Model
{
     ...
     * @return array
     */
    public static function check2($version, $appid = '10000')
    {
        $versionlist = self::where(['status' => 'normal', 'box_app_ids' => $appid])->where('newversion', '<>' , $version)->order('weigh desc,id desc')->select();
        foreach ($versionlist as $k => $v) {
            if ($v['status'] == 'normal') {
                $updateversion = $v;
                break;
